...
export async function getAssociatedVisitorsGroup( webUrl: string, ): Promise<IGroupResults> {
  ...
  try {
    ...
    if ( check4Gulp() === true ) { console.log( `fps-Pnp2 SUCCESS: getAssociatedVisitorsGroup ~ 23`, group, group ) ; };

    return { group: group, e: null, status: 'Success' }

  } catch (e) {

    if ( check4Gulp() === true ) { console.log( `fps-Pnp2 ERROR: getAssociatedVisitorsGroup ~ 29`, e ) ; };
