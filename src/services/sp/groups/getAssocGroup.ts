...
export async function getAssociatedGroup( webUrl: string, groupType: IAssociatedGroupType ): Promise<IGroupResults> {
  ...
  try {
    ...
    if ( groupType === 'Owners' ) {
      ...
      if ( check4Gulp() === true ) { console.log( `fps-Pnp2 SUCCESS: getAssociatedGroup ~ 23`, groupType, group ) ; };
      ...
      if ( check4Gulp() === true ) { console.log( `fps-Pnp2 SUCCESS: getAssociatedGroup ~ 27`, groupType, group ) ; };
      return { group: group, e: null, status: 'Success' }

    } else if ( groupType === 'Visitors' ) {
      let group: ISiteGroupInfo = await thisWeb.associatedVisitorGroup();
      if ( check4Gulp() === true ) { console.log( `fps-Pnp2 SUCCESS: getAssociatedGroup ~ 33`, groupType, group ) ; };
      return { group: group, e: null, status: 'Success' }
    ...
    if ( check4Gulp() === true ) { console.log( `fps-Pnp2 ERROR: getAssociatedGroup ~ 44`, e ) ; };
