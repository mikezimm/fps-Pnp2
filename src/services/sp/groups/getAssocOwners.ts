...
export async function getAssociatedOwnersGroup( webUrl: string, ): Promise<IGroupResults> {
  ...
  try {
    ...
    if ( check4Gulp() === true ) { console.log( `fps-Pnp2 SUCCESS: getAssociatedOwnersGroup ~ 23`, group, group ) ; };

    return { group: group, e: null, status: 'Success' }

  } catch (e) {

    if ( check4Gulp() === true ) { console.log( `fps-Pnp2 ERROR: getAssociatedOwnersGroup ~ 29`, e ) ; };
